...
#define SERVER_NAME "10.10.141.68"
#define SERVER_PORT 5000
#define LOGID "KSJ_ARD"
...
char sendId[10] = "KSJ_STM";
char sendBuf[CMD_SIZE];
char lcdLine1[17] = "Smart IoT By KSJ";
...
void setup() {
  ...
  pinMode(ledArray[0], OUTPUT);  //R
  pinMode(ledArray[1], OUTPUT); //G  
  pinMode(ledArray[2], OUTPUT); //B 
  analogWrite(ledArray[0],255); //r
  analogWrite(ledArray[1],255); //g       
  analogWrite(ledArray[2],255); //b
#ifdef DEBUG
...
void loop() {
  ...
  if (timerIsrFlag) //1초에 한번씩 실행
  {
    ...
    if (!(secCount % 5)) //5초에 한번씩 실행
    {
      ...
      {
        cdsFlag = false;
        sprintf(sendBuf, "[%s]SENSOR@%d@%d@%d\r\n", sendId, cds, (int)humi, (int)temp);
        client.write(sendBuf, strlen(sendBuf));
        client.flush();
        digitalWrite(LED_LAMP_PIN, LOW);     // LED 상태 변경
      ...
      } else if ((cds < 50) && !cdsFlag)
      {
        ...
        client.write(sendBuf, strlen(sendBuf));
        client.flush();
        digitalWrite(LED_LAMP_PIN, HIGH);     // LED 상태 변경
      }*/
...
void socketEvent()
{
  ...
  while (pToken != NULL)
  {
    ...
    Serial.println(pArray[3]);
    Serial.println(pArray[4]);
  ...
  else if (!strncmp(pArray[1], " Alr", 4)) //Already logged
  {
    ...
    if (!strncmp(pArray[1], "LAMPON",6)) {
      digitalWrite(LED_LAMP_PIN, HIGH);
    }
    else if (!strncmp(pArray[1], "LAMPOFF",7)) {
      digitalWrite(LED_LAMP_PIN, LOW);
    }
    ...
    if (!strncmp(pArray[1], "PLUGON",6)) {

      int newR = random(2) * 255; // 랜덤한 값으로 R 색상 생성
      int newG = random(2) * 255; // 랜덤한 값으로 G 색상 생성
      int newB = random(2) * 255; // 랜덤한 값으로 B 색상 생성

      analogWrite(ledArray[0], newR); //r
      analogWrite(ledArray[1], newG); //g       
      analogWrite(ledArray[2], newB); //b
    }else if (!strncmp(pArray[1], "PLUGOFF",7)) {
      analogWrite(ledArray[0],255); //r
      analogWrite(ledArray[1],255); //g       
      analogWrite(ledArray[2],255); //b
    }
  ...
  else if (!strcmp(pArray[1], "LEDCOLOR")) {

    red = atoi(pArray[2]);
    green = atoi(pArray[3]);
    blue = atoi(pArray[4]);
    analogWrite(ledArray[0],(unsigned char)(~red)); //r
    analogWrite(ledArray[1],(unsigned char)(~green)); //g       
    analogWrite(ledArray[2],(unsigned char)(~blue)); //b
