...
public class Database extends SQLiteOpenHelper {
    private static String DATABASE_NAME = "rast";
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        String[] insertArgs = new String[]{"KSJ_ARD","LAMP","OFF"};
        insertRecordParam(sqLiteDatabase,insertArgs);
        insertArgs = new String[]{"KSJ_ARD","PLUG","OFF"};
        insertRecordParam(sqLiteDatabase,insertArgs);
    }
    @Override
...
//        Log.d("Database","strDate:"+strDate+" strTime:"+strTime+",node:"+node+",sensor:"+sensor+",value:"+value+ ", rowPosition:"+rowPosition);
    ...
    public int updateRecordParam(SQLiteDatabase db, String updateArgs[]) {
        ...
        String value="";
        int rowPosition = 0;
        if(updateArgs[0].equals("KSJ_ARD")) {
            node = updateArgs[0];
            sensor = updateArgs[1];
            value = updateArgs[2];
...
//        String date,time,node,sensor,value;
        Log.d("executeRawQueryParam",arg);
        String SQL = "select * " + " from " + TABLE_NAME + " where sensor == ? ";
        Cursor cursor = db.rawQuery(SQL, whereArgs);
        int recordCount = cursor.getCount();
        Log.d("executeRawQueryParam",""+recordCount);
